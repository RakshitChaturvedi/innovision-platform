...
@router.post("/login")
async def login(email: str, password: str, response: Response, db: AsyncSession = Depends(...)):
...
@router.post("/refresh")
async def refresh(session_id: str, refresh_token: str, db: AsyncSession = Depends(...)):
...
@router.post("/logout")
async def logout(user: dict = Depends(get_current_user), db: AsyncSession = Depends(...)):
